...
import { containerFields, type FormContainer, type FormField } from '@tramites/form-contracts';
...
function defaultRow(container: FormContainer): Record<string, unknown> {
  return Object.fromEntries(containerFields(container).map((field) => [field.fieldName, field.defaultValue ?? (field.type === 'checkbox' ? false : '')]));
}

function RepeaterCell({ field, control, name, enabled = true }: { field: FormField; control: Control<FormValues>; name: string; enabled?: boolean }) {
    ...
    <Controller
      ...
      render={({ field: controllerField, fieldState }) => (
        <div className={`repeater-cell-control${fieldState.error ? ' has-error' : ''}`}>
          {renderField({ field, controllerField: controllerField as never, enabled, options: field.options ?? [] })}
...
export function DynamicRepeater({ container, control, enabled = true }: DynamicRepeaterProps) {
  ...
  const columns = containerFields(container);
        ...
        {fields.map((row, rowIndex) => (
          <div className="repeater-row" role="row" key={row.id}>
            {columns.map((field) => (
              ...
              <RepeaterCell field={field} control={control} name={`${fieldName}.${rowIndex}.${field.fieldName}`} enabled={enabled} />
              </div>
            ))}
